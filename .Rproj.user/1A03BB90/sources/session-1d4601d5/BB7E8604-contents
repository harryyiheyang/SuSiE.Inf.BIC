#' Fast Fine-Mapping and Estimation of Causal Variants Using SuSiE
#'
...
df=Lvec[i]
Theta_alpha=matrixInverse(R+LD2*mean(alpha^2))
res=z-matrixVectorMultiply(R,beta)
Bicvec[i]=log(sum(res*matrixVectorMultiply(Theta_alpha,res)))+log(m)/m*df
Bbeta[,i]=beta
Balpha[,i]=alpha
Bvar.inf[i]=var.inf
Bpv[i]=pv
...
pip=Bpip[,istar]
fit=Bfit[[istar]]
return(list(eta=(alpha+beta)/sqrt(n),beta=beta/sqrt(n),alpha=alpha/sqrt(n),var.inf=var.inf,pv=pv,pip=pip,fit.susie=fit,L=Lvec[istar]))
}
